    ...
    <ul class="v-breadcrumbs breadcrumbs theme--light">
      ...
      <li>
        <div class="v-breadcrumbs__item v-breadcrumbs__item--disabled">
          {{ activity.senario_template_name }}
        </div>
      </li>
    </ul>
    <h2 class="mb-4 grey--text mt-5">{{ activity.senario_name }}</h2>
    <v-form ref="form" lazy-validation v-model="valid">
      <v-card elevation="2">
        <h3 class="pa-3">アクティビティ編集</h3>
        <v-divider></v-divider>
        <div class="pa-4">
          <h3 class="font-weight-bold grey--text">対象セグメント</h3>
          <h1 style="font-size: 29px">{{ this.activity.segment_name }}</h1>
            ...
            <ul class="radio-type">
              <li>
                <v-radio
                    ...
                    :value="1"
                    :disabled="activity.schedule_type != 1"
                ></v-radio>
              </li>
              <li>
                <v-radio
                    ...
                    color="light-green accent-4"
                    :value="2"
                    :disabled="activity.schedule_type != 2"
                ></v-radio>
              </li>
              <li>
                <v-radio
                    ...
                    color="light-green accent-4"
                    :value="3"
                    :disabled="activity.schedule_type != 3"
                ></v-radio>
              </li>
            </ul>
          </v-radio-group>
          ...
          <!-- Radio 1 -->
          <div
              v-if="activity.schedule_type == 1"
              class="grey lighten-4 rounded pa-6"
          >
            ...
            <!--  -->
            <v-row class="mt-0 mb-0" v-if="isTimeUnitTyeEqualThree">
              <v-col cols="12" md="6" class="mb-0 pb-0">
                <span>送信タイミング</span>
                <v-radio-group v-model="activity.r1_send_time_type" style="display:block">
                    ...
                    <v-row>
                      ...
                      <v-col md="9" class="mb-0">
                        <v-select
                            ...
                            outlined
                            required
                            :disabled="isDisableSendTimeTypeEqualThree"
                            v-model="activity.r1_send_time_type_3"
                            :rules="selectBoxTimeRules"
                        >
                        </v-select>
                      </v-col>
                      <v-col md="1" class="mb-0">
                    ...
                    <v-row>
                      ...
                      <v-col md="4" class="mb-0">
                        <v-select
                            ...
                            single-line
                            outlined
                            :disabled="isDisableSendTimeTypeEqualOne"
                    ...
                    <v-row>
                      ...
                      <v-col md="2" class="mb-0">
                        ...
                        <v-select
                            ...
                            single-line
                            outlined
                            :disabled="isDisableSendTimeTypeEqualTwo"
            ...
            <v-row class="mt-0 mb-0" v-if="isTimeUnitTyeEqualTwo">
              <div>
                <v-col cols="12" md="6" class="mb-0 pb-0">
                  <span>曜日</span>
            ...
            <!-- display when r1_time_unit_type !== 2 -->
            <v-row
                class="mt-0 mb-0"
                v-if="isTimeUnitTyeEqualTwo || isTimeUnitTyeEqualOne"
            >
              ...
              <template slot-scope="{ nodeData }">
                ...
                <div class="mail" v-if="!nodeData.actionType">
                  ...
                  <v-btn
                      icon
                      class="delete-button"
                      @click="showDeletePopup(nodeData, 'MAIL')"
                      v-if="nodeData['id'] != 1"
                  >
                ...
                >
                  ...
                  <v-btn
                      icon
                      class="delete-button"
                      @click="showDeletePopup(nodeData, 'ACTION')"
                  >
                    <v-icon>mdi-delete</v-icon>
                  </v-btn>
                </div>
      ...
      <v-card>
        ...
        <v-card-actions class="justify-center align-center pb-6">
          ...
          <v-btn color="success"
                 class="light-green accent-4 justify-center align-center font-weight-bold"
                 width="120px" rounded
                 @click="deleteNode(itemToDelete, actionType)">
            削除
          </v-btn>
        </v-card-actions>
      </v-card>
...
import DialogMail from "@/components/senario/activity/DialogMailUpdate.vue";
...
import {
  ...
  MONTH,
  DAY,
  SCHEDULE_TYPE_TYPE_3, SEND_TIME_TYPE_1, SEND_TIME_TYPE_2, SEND_TIME_TYPE_3, HOUR_LIST,
  isDuplicateActivity
...
export default {
  ...
  data: () => ({
    ...
    selectBoxDayInWeekRules: [(v) => (!!v) || "曜日は必須項目です"],
    isDisable: true,
    mailData: {},
    actionDeleted: [],
    mailDeleted: [],
    mailOld: [],
    actionOld: [],
    isTimeUnitTyeEqualTwo: false,
    isTimeUnitTyeEqualThree: false,
    isTimeUnitTyeEqualOne: false,
    isDisableSendTimeTypeEqualOne: true,
    isDisableSendTimeTypeEqualTwo: true,
    isDisableSendTimeTypeEqualThree: true,
    itemToDelete: null,
    actionType: null,
  ...
  methods: {
      ...
      try {
        ...
        try {
          ...
          if (isDuplicateActivity()) {
            this.activity.name = result.data.data["activity"].name + " - copy";
          }

          this.activity.schedule_setting = JSON.parse(
              result.data.data["activity"].schedule_setting
          );
          this.setDataScheduleActivity(result.data.data["activity"]);
          this.changeTimeUnitType(this.activity.schedule_setting.time_unit_type);
          this.changeSendTimeType(this.activity.schedule_setting.send_time_type);

          this.ds = result.data.data["actionAndMail"];
          this.mailOld = result.data.data["mail"];
          this.actionOld = result.data.data["action"];
        } catch (err) {
          let stt = err.response.status;
          if (stt == 500) return (this.errors = [err.response.data.response_message]);
    ...
    mappingDataScheduleTypeOne(schedule_setting) {
      this.activity.schedule_type = schedule_setting.schedule_type;
      this.activity.r1_time_unit_type = schedule_setting.time_unit_type;
      this.activity.r1_time_unit_value = schedule_setting.time_unit_value;
      this.activity.r1_start_date = schedule_setting.start_date;
      this.activity.r1_end_date = schedule_setting.end_date;
      if (schedule_setting.time_unit_type == DAY) {
        this.activity.r1_send_time_unit_type_1 =
            schedule_setting.send_time;
      } else if (schedule_setting.time_unit_type == WEEK) {
        this.activity.r1_day_in_week = schedule_setting.day_in_week.split(",");
        this.activity.r1_send_time_unit_type_1 =
            schedule_setting.send_time;
      } else {
        this.activity.r1_send_time_type = schedule_setting.send_time_type;
        if (schedule_setting.send_time_type == 1) {
          this.activity.r1_day_in_month = schedule_setting.day_in_month;
          this.activity.r1_send_time_type_1 = schedule_setting.send_time;
        } else if (schedule_setting.send_time_type == 2) {
          this.activity.r1_send_time_type_2 = schedule_setting.send_time;
          this.activity.r1_send_time_day = schedule_setting.send_time_day;
          this.activity.r1_kind_of_day = schedule_setting.kind_of_day;
        } else {
          this.activity.r1_send_time_type_3 = schedule_setting.send_time;
        }
      }
    },
    mappingDataScheduleTypeTwo(schedule_setting) {
      this.activity.r2_target_data = schedule_setting.target_data;
      this.activity.r2_start_date = schedule_setting.start_date;
      this.activity.r2_end_date = schedule_setting.end_date;
      this.activity.r2_send_time_before_after_type =
          schedule_setting.send_time_before_after_type;
      this.activity.r2_send_time_before_after =
          schedule_setting.send_time_before_after;
      this.activity.r2_send_time = schedule_setting.send_time;
    },
    setDataScheduleActivity(data) {
      let schedule_setting = this.activity.schedule_setting;
      this.changeTimeUnitType(schedule_setting.time_unit_type);
      this.changeSendTimeType(schedule_setting.send_time_type);

      if (data.schedule_type == SCHEDULE_TYPE_TYPE_1) {
        this.mappingDataScheduleTypeOne(schedule_setting);
      } else if (data.schedule_type == SCHEDULE_TYPE_TYPE_2) {
        this.mappingDataScheduleTypeTwo(schedule_setting);
      } else {
        this.activity.r3_send_time = schedule_setting.send_time;
        this.activity.r3_send_date = schedule_setting.send_date;
      }
    },
    deleteMail: function () {
    ...
    showDeletePopup(node, actionType) {
      this.itemToDelete = node;
      this.actionType = actionType;
      this.showDeleteTemplate = true;
    },
    deleteNode(node, type) {
      this.showDeleteTemplate = false;
      if (type == "MAIL" && node.mail_id) {
        this.mailDeleted.push(node.mail_id);
      }
      if (type == "ACTION" && node.action_id) {
        this.actionDeleted.push(node.action_id);
      }
      this.deleteNodeFromTree(this.ds, node.id);
    },
    ...
    getTarget() {
      let data = getTargetDatas(this.getSenarioTemplateType());
    ...
    async submitForm() {
      try {
        ...
        let data = {
          ...
          stage_id: this.activity.stage_id,
          senario_id: this.activity.senario_id,
          segments: this.activity.sub_segment_ids.join(","),
          action_and_mail: JSON.stringify(this.ds),
          action_old: this.actionOld,
          mail_old: this.mailOld,
        };
        try {
          if (isDuplicateActivity()) {
            await ActivityService.store(data);
          } else {
            await ActivityService.updateById(activityId, data);
          }
          this.alertSuccess();
          this.errors = [];
    ...
    getDataScheduleTypeEqualTwo() {
      return {
        ...
        schedule_setting: this.activity.schedule_type,
      };
    },
    ...
    changeTimeUnitType(value) {
      if (value == WEEK) {
        this.isTimeUnitTyeEqualTwo = true;
        this.isTimeUnitTyeEqualThree = false;
        this.isTimeUnitTyeEqualOne = false;
      } else if (value == MONTH) {
        this.isTimeUnitTyeEqualTwo = false;
        this.isTimeUnitTyeEqualThree = true;
        this.isTimeUnitTyeEqualOne = false;
      } else if (value == DAY) {
        this.isTimeUnitTyeEqualOne = true;
        this.isTimeUnitTyeEqualTwo = false;
        this.isTimeUnitTyeEqualThree = false;
      }
    },
    changeSendTimeType(value) {
      if (value == SEND_TIME_TYPE_1) {
        this.isDisableSendTimeTypeEqualOne = false;
        this.isDisableSendTimeTypeEqualTwo = true;
        this.isDisableSendTimeTypeEqualThree = true;
      } else if (value == SEND_TIME_TYPE_2) {
        this.isDisableSendTimeTypeEqualOne = true;
        this.isDisableSendTimeTypeEqualTwo = false;
        this.isDisableSendTimeTypeEqualThree = true;
      } else if (value == SEND_TIME_TYPE_3) {
        this.isDisableSendTimeTypeEqualOne = true;
        this.isDisableSendTimeTypeEqualTwo = true;
        this.isDisableSendTimeTypeEqualThree = false;
      }
    },
