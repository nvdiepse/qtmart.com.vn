...
app()->booted(function () {
    ...
    if (is_plugin_active('simple-slider')) {
        ...
        add_filter(SHORTCODE_REGISTER_CONTENT_IN_ADMIN, function ($data, $key, $attributes) {
            if ($key == 'simple-slider' && is_plugin_active('ads')) {
                ...
                return $data . Theme::partial('shortcodes.select-ads-admin-config', compact('ads', 'attributes'));
            }
    ...
    if (is_plugin_active('ads')) {
        ...
        function display_ads_advanced(?string $key, array $attributes = []) :?string
        {
            ...
            $image = Html::image(image_placeholder(), $ads->name, ['class' => 'lazyload', 'data-src' => RvMedia::getImageUrl($ads->image)])->toHtml();
